<?php
require_once __DIR__ . "/../../../config.php";
require_once __DIR__ . "/../../fce.php";
require_once __DIR__ . "/../../variableCheck.php";

$conn = mysqli_connect($dbServer,$dbUzivatel,$dbHeslo,$dbDb);
if (!$conn) { echo "Nem�me data!"; return; }
mysqli_query($conn, "SET NAMES 'utf8mb4'");

// 30min ko�e
$sql = "
  SELECT 
    FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(date_time)/1800)*1800) AS bucket,
    AVG(temperature)           AS temperature,
    AVG(temperature_apparent)  AS temperature_apparent,
    AVG(humidity)              AS humidity,
    AVG(dew_point)             AS dew_point,
    AVG(rain_rate)             AS rain_rate,   -- mm/h
    MAX(rain_weekly)           AS rain_weekly, -- mm (kumulativ za t�den)
    AVG(pressure_QNH)          AS pressure_QNH,
    AVG(exposure)              AS exposure,
    AVG(wind_speed)            AS wind_speed
  FROM history_cron_padarovice
  WHERE date_time >= NOW() - INTERVAL 7 DAY
  GROUP BY bucket
  ORDER BY bucket ASC";
$res = mysqli_query($conn, $sql);
mysqli_close($conn);

if (!$res || mysqli_num_rows($res) <= 0) { echo "Nem�me data!"; return; }

$labels=[]; $y1=[]; $y2=[]; $y3=[]; $y4=[]; $y5=[]; $y6=[]; $y7=[]; $y8=[]; $yCumWeek=[];
while ($r = mysqli_fetch_assoc($res)) {
  $labels[]   = $r['bucket'];
  $y1[]       = round(jednotkaTeploty((float)$r['temperature'], $u, 0), 1);
  $y2[]       = round(jednotkaTeploty((float)$r['temperature_apparent'], $u, 0), 1);
  $y3[]       = round((float)$r['humidity'], 1);
  $y4[]       = round(jednotkaTeploty((float)$r['dew_point'], $u, 0), 1);
  $y5[]       = is_null($r['rain_rate'])    ? 0 : (float)$r['rain_rate'];         // mm/h
  $yCumWeek[] = is_null($r['rain_weekly'])  ? 0 : round((float)$r['rain_weekly'], 1); // mm
  $y6[]       = round(((float)$r['pressure_QNH']) * ($ut === 'mm' ? 0.750062 : 1), 1); // hPa → ev. mmHg
  $y7[]       = round((float)$r['exposure'], 1);
  $y8[]       = round(((float)$r['wind_speed']) * ($uv === 'm' ? 1/3.6 : 1), 1); // km/h → ev. m/s
}
$jednotkaVit = jednotkaVitrSymbol($uv);
$jednotkaTl  = jednotkaTlakSymbol($ut);

// �t�tky a odd�lova�e dn�
$plotLines = []; $labelsOut = []; $prevDay = null;
foreach ($labels as $i => $ts) {
  $day = substr($ts, 0, 10);
  if ($prevDay !== null && $day !== $prevDay) { $labelsOut[] = formatDnu($day); $plotLines[] = $i; }
  else { $labelsOut[] = substr($ts, 11, 5); }
  $prevDay = $day;
}
$plotLinesOut = implode(',', array_map(fn($v)=>"{ color:'lightgrey', dashStyle:'solid', value:$v, width:1 }",$plotLines));
$jednotka = jednotkaSymbol($u);
?>
<script>
jQuery(function($){
  var chart = new Highcharts.Chart({
    chart: { renderTo:'graf-5-dni', zoomType:'x', backgroundColor:'#ffffff', borderRadius:0 },
    credits: { enabled:false },
    xAxis: {
      categories: ['<?= implode("','", array_map('htmlspecialchars',$labelsOut)) ?>'],
      labels: { rotation:-45, align:'right', step:10 },
      plotLines: [<?= $plotLinesOut ?>]
    },
    yAxis: [{
      // 0 � teploty
      labels: { formatter:function(){ return this.value + ' <?= $jednotka ?>'; }, style:{color:'#c4423f'} },
      title: { text:null, style:{color:'#c4423f'} },
      opposite:false
    },{
      // 1 � vlhkost
      title:{ text:null, style:{color:'#33cccc'} },
      labels:{ formatter:function(){ return this.value + ' %'; }, style:{color:'#33cccc'} },
      opposite:true, max:100, ceiling:100
    },{
      // 2 � sr�ky intenzita (mm/h)
      title:{ text:null, style:{color:'#0066ff'} },
      labels:{ formatter:function(){ return this.value + ' mm/h'; }, style:{color:'#0066ff'} },
      opposite:true
    },{
      // 3 � tlak
      title:{ text:null, style:{color:'#800000'} },
      labels:{ formatter:function(){ return this.value + ' <?= $jednotkaTl ?>'; }, style:{color:'#800000'} },
      opposite:true
    },{
      // 4 � osvit
      title:{ text:null, style:{color:'#999900'} },
      labels:{ formatter:function(){ return this.value + ' W'; }, style:{color:'#999900'} },
      opposite:true
    },{
      // 5 � v�tr
      title:{ text:null, style:{color:'#3399ff'} },
      labels:{ formatter:function(){ return this.value + ' <?= $jednotkaVit ?>'; }, style:{color:'#3399ff'} },
      opposite:true
    },{
      // 6 � sr�ky kumulativ t�dn� (mm)
      title:{ text:null, style:{color:'#0047b3'} },
      labels:{ formatter:function(){ return this.value + ' mm'; }, style:{color:'#0047b3'} },
      opposite:true
    }],
    tooltip: {
      shared:false,
      formatter: function () {
        var unit = {
          '<?= $lang['teplota'] ?>'         : ' <?= $jednotka ?>',
          '<?= $lang['pocteplotazkratka'] ?>': ' <?= $jednotka ?>',
          '<?= $lang['kumsrazkytyden'] ?>'  : ' mm',
          '<?= $lang['vlhkost'] ?>'         : ' %',
          '<?= $lang['rosnybod'] ?>'        : ' <?= $jednotka ?>',
          '<?= $lang['srazky'] ?>'          : ' mm/h',
          '<?= $lang['tlak'] ?>'            : ' <?= $jednotkaTl ?>',
          '<?= $lang['osvit'] ?>'           : ' W',
          '<?= $lang['vitr'] ?>'            : ' <?= $jednotkaVit ?>'
        }[this.series.name] || '';
        return '<b>' + this.x + '</b><br />' +
               '<span style="color:'+this.series.color+'">\u25CF</span> ' +
               this.series.name + ': <b>' + this.y + unit + '</b>';
      },
      crosshairs: true
    },
    legend: { layout:'horizontal', align:'left', x:6, verticalAlign:'top', y:-5, floating:true, backgroundColor:'#FFFFFF' },

    // ��� po�ad� s�ri�: teploty � kumulativ t�dne � ostatn� � intenzita sr�ek ���
    series: [{
      name:'<?= $lang['teplota'] ?>',
      type:'spline', color:'#c4423f', yAxis:0,
      data:[<?= implode(',', $y1) ?>], marker:{enabled:false}
    },{
      name:'<?= $lang['pocteplotazkratka'] ?>',
      type:'spline', color:'#990099', yAxis:0,
      data:[<?= implode(',', $y2) ?>], marker:{enabled:false}, visible:false
    },{
      name:'<?= $lang['srazky'] ?>',
      type:'column', color:'#0066ff', yAxis:2,
      data:[<?= implode(',', $y5) ?>], marker:{enabled:false}
    },{
      name:'<?= $lang['kumsrazkytyden'] ?>',
      type:'spline', color:'#0047b3', yAxis:6,
      data:[<?= implode(',', $yCumWeek) ?>], marker:{enabled:false}, visible:false
    },{
      name:'<?= $lang['vlhkost'] ?>',
      type:'spline', color:'#33cccc', yAxis:1,
      data:[<?= implode(',', $y3) ?>], marker:{enabled:false}, visible:false
    },{
      name:'<?= $lang['rosnybod'] ?>',
      type:'spline', color:'#009933', yAxis:0,
      data:[<?= implode(',', $y4) ?>], marker:{enabled:false}, visible:false
    },{
      name:'<?= $lang['tlak'] ?>',
      type:'spline', color:'#800000', yAxis:3,
      data:[<?= implode(',', $y6) ?>], marker:{enabled:false}, visible:false
    },{
      name:'<?= $lang['osvit'] ?>',
      type:'spline', color:'#e6e600', yAxis:4,
      data:[<?= implode(',', $y7) ?>], marker:{enabled:false}, visible:false
    },{
      name:'<?= $lang['vitr'] ?>',
      type:'spline', color:'#3399ff', yAxis:5,
      data:[<?= implode(',', $y8) ?>], marker:{enabled:false}, visible:false
    }]
  });

  $(".tabs > li").on('click', function(){ chart.reflow(); });
});
</script>
